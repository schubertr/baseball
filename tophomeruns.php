...
<style>
h1.titles{width: 200px; margin-left: auto; margin-right: auto; white-space: nowrap}
...
$stmt = $db->query("SELECT *, HR FROM batting ORDER BY HR DESC LIMIT 25");
$result=$stmt->fetchAll(PDO::FETCH_ASSOC);

echo"<h1 class=titles>Top Home Runs</h1>";
...
echo "<th class = r style= 'padding: 0 0 0 30px; white-space: nowrap;'>Home Runs</th>";
...
for($i = 0 ; $i < $len ; $i++)
{
	...
	echo "<td style='text-align: right;'>$HR</td>";
	echo "</tr>";
...
$stmt = $db->query("SELECT playerID, SUM(HR) as HR FROM batting GROUP BY playerID ORDER BY SUM(HR) DESC LIMIT 25");
$result=$stmt->fetchAll(PDO::FETCH_ASSOC);
